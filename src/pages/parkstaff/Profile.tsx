...
const ParkStaffProfile = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      navigate('/login');
    } else if (user && user.role !== 'park-staff') {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${API_URL}/parkstaff/profile`, {
      ...
      if (avatarFile) {
        ...
        const avatarResponse = await axios.post(`${API_URL}/parkstaff/avatar`, formData, {
          headers: {
  ...
  const handleChangePassword = async () => {
    ...
    setIsSaving(true);
    try {
      await axios.put(`${API_URL}/parkstaff/password`, {
        currentPassword,
        newPassword,
        confirmPassword,
      }, {
  ...
  const handleDeleteAccount = async () => {
    ...
    setIsSaving(true);
    try {
      await axios.delete(`${API_URL}/parkstaff/account`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      logout();
      navigate('/login');
...
export default ParkStaffProfile;